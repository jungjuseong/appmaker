...
$(document).ready(function(){
	//first access, table arrangement
	...
	//first access, table arrangement - end
		...
		$.ajax({
			...
			success:function(result){
				switch(result){
					case 0 :
						//message : 등록 하시겠습니까?
						if(confirm("<spring:message code='group.write.014' />")){
...
function cancelResist(){
	//message : 지금까지 입력한 내용이 사라집니다. 취소하시겠습니까?
...
<div id="wrap" class="sub_wrap">
	...
	<div id="container">
		...
		<c:set var="curi" value="${requestScope['javax.servlet.forward.request_uri']}" />
			...
			<sec:authorize access="hasAnyRole('ROLE_USER','ROLE_COMPANY_MEMBER')">
			<h2><spring:message code='group.write.001' /></h2>
			</sec:authorize>
			<sec:authorize access="hasAnyRole('ROLE_ADMIN_SERVICE')">
			<h2><spring:message code='group.write.001.member' /></h2>
			</sec:authorize>
			
			<form id="group_write_f" name="group_write_f" method="post" action="/man/group/write.html?currentPage=${param.currentPage}&searchValue=${param.searchValue}" >
				...
				<div class="section fisrt_section">
					...
					<div class="table_area">
						<table id="menu_list" class="col_table">
							<colgroup>
							...
							<tr class="group">
								...
								<sec:authorize access="hasAnyRole('ROLE_USER','ROLE_COMPANY_MEMBER')">
									<td colspan="3" style="text-align:left;"><input id="groupName" name="groupName" type="text" style="width:50%;" class="line_right"></td>
									<input type="hidden" id="memberGb" name="memberGb" value=""/>
								</sec:authorize>
								<sec:authorize access="hasAnyRole('ROLE_ADMIN_SERVICE')">
									<td colspan="2" style="text-align:left;"><input id="groupName" name="groupName" type="text" style="width:70%;" class="line_right"></td>
									<td>
										<div>
											<input name="memberGb" id="company_member" type="radio" value="1" checked="checked" > <label for="company_member"><spring:message code='member.join.051' /></label>&nbsp;&nbsp;&nbsp;
											<input name="memberGb" id="private_member" type="radio" value="2"> <label for="private_member"><spring:message code='member.join.052' /></label>
										</div>
									</td>
								</sec:authorize>
					...
					<div class="btn_area_bottom tCenter">
						<a id="registBtn" href="#registBtn_;" class="btn btnL btn_red"><spring:message code='group.write.010' /></a>
