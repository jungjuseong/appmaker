...
//app_modify.jsp : app_modify, distribute_req, distribute_restore에서 사용
...
//	completGbStatus('load');		20180411 : lsy - complet_gb radio delete
	...
	if('${app.completGb}' == '1'){//배포 승인(완료)
		textItemDisabled();
		$("[name=useGb]").attr('disabled', 'disabled');
		$("[name=installGb]").attr('disabled', 'disabled');
	}else if('${app.completGb}' == '2' || '${app.completGb}' == '4'){//테스트중 + 배로 반려(반려 내역 관리 기능)
		if('${app.completGb}' == '2'){
			$("[name=useGb]").prop('checked', false).attr('disabled', 'disabled');
			$("[name=installGb]").prop('checked', false).attr('disabled', 'disabled');
		}else if('${app.completGb}' == '4'){
			$("[name=useGb]").attr('disabled', 'disabled');
			$("[name=installGb]").attr('disabled', 'disabled');
		}
		distributeCouponDisabled();
	}else if('${app.completGb}' == '3'){//배포 요청중
		textItemDisabled();
		$("[name=loginGb]").attr('disabled', 'disabled');
		$("[name=useUserGb]").attr('disabled', 'disabled');
		if('${app.distrGb}' == null || '${app.distrGb}' == ''){//배포범위/쿠폰 처음 설정할 때
			distributeCouponDefault();
		}
		$("#modifyDeleteSection").hide();
	}
	//20180411 : lsy - case complet_gb, able/disabled process - end
	...
	$("#viewLog").click(function(){
		winname = window.open("/app/log/list.html?page=1&storeBundleId=${app.storeBundleId}&dateIsCheck=2","haha","width=850 height=550 menubar=no status=no");
	});
	...
	$('input[type=file][name=iconFile], input[type=file][name=captureFile]').click(function(e){
		//20180409 : lsy - inapp complet, not modify info
		if('${app.completGb}' == '2'){
			...
			alert("<spring:message code='app.modify.text77' />");
			return false;
		}
		//20180409 : lsy - inapp complet, not modify info - end
	});
	...
	$('#templateNameBtn').click(function(){
		//20180409 : lsy - app complet, not modify info
		if('${app.completGb}' == '2'){
			var winWidth = 670;
			var winHeight = 740;
			var winPosLeft = (screen.width - winWidth)/2;
		...
		//20180409 : lsy - app complet, not modify info - end
	});	
	...
	$('#inAppContentsSearch').click(function(){
		...
		// testModify
		if("${param.test}" == 'true'){
			var url = "/app/inapp/list.html?storeBundleId="+ $("#storeBundleId1").val() + $("#storeBundleId2").val() + "&test="+"${param.test}";
		}else{
			// baseCode
			var url = "/app/inapp/list.html?storeBundleId="+ $("#storeBundleId1").val() + $("#storeBundleId2").val() + "&isAvailable=false&searchType=inappName&searchValue=";
		}
	...
	$(document).on("click", "#registBtn, #distributeReq, #distributeComplet", function(){
		//post 형식
...
/* 			if(isVaid&&$('[name=provSeq]').size()==0){
			...
			if(isVaid){
				if($('[name=ostype]').val()!='4'){
					if(!$('.prov_area').find('ul').find('li').length>0){
			...
			//20180412 : lsy - not create app(column : app_resultCode), not distributeReq
			if("${app.app_resultCode}" != "1"){
		...
		if($("#restoreReason").css("display") == "none"){
			...
			$("#restoreReason").css("display", "block");
 			...
 			if(confirm("<spring:message code='distribute.restore.003' />")){
				window.location.href="/distribute/restore.html?referer="+$("#referer").val()+"&type=app&existSeq="+$("#appSeq").val()+"&restoreText="+$("#restoreReasonText").val();
 			}
		}		
	});
	...
	$(document).on("click", "#distributeCancle, #cancleBtn", function(){
		if($(this).attr("id") == "cancleBtn"){
			...
			if($("#referer").val() == "list"){
				...
				window.location.href="/distribute/list.html?page=1&objectGb=app&myListGb=${myListGb}";
			}	
		}
	});
	...
	$(document).on("click", "#restoreBreakdown", function(){
		...
		var winPosLeft = (screen.width - winWidth)/2;
		var winPosTop = (screen.height - winHeight)/2;
		var url = "/app/restoreView.html?type=app&existSeq="+$("#appSeq").val()+"&name="+$("#appName").val();		
		var opt = "width=" + winWidth + ", height=" + winHeight + ", top=" + winPosTop + ", left=" + winPosLeft + ", scrollbars=No, resizeable=No, status=No, toolbar=No";
...
*/
	...
	//고정, 최대, 제한없을음 누를때 http://www.apple.com/kr/
...
/*	20180411 : lsy - complet_gb radio delete
	function completGbStatus(type){
		if("${app.app_resultCode}" == "2" && $('input:radio[name=completGb]:checked').val() == 1){
			...
			}//temp - 배포 완성 시 history 넣을 구분 값 넣어주는 부분(수정 필요!!!)
...
*/
	...
	$("#d_end_date").mousedown(function(e){
		if($("#d_start_date").val() == null || $("#d_start_date").val() == ""){
	...
	$("#c_end_date").mousedown(function(e){
		if($("#c_start_date").val() == null || $("#c_start_date").val() == ""){
			//message : 시작 날짜를 입력해주십시오.
			alert("<spring:message code='contents.modify.039' />");
			 e.preventDefault(); 
		} 
	...
	$("#c_start_date").change(function(){
		...
		//HTML에서 가져온 날자 값
		var inputDate = $("#d_start_date").val();
	...
	$("#appRequest").click(function(){
		...
		var templateSeq  = $("[name = templateSeq]").val();
		var appContentsAmtJ = $("[name = appContentsAmt]").val();
		var appNameJ = $("[name=appName]").val();
		var fileNameJ = $("[name=fileName]").val();
		var appContentsGbJ = $("[name=appContentsGb]:checked").val();
		...
		if(typeof templateName != 'undefined' && templateName.length > 0){
			$.ajax({
	            ...
	            data:{	
					...
					iconSaveFile : $("[name=iconSaveFile]").val()
	            },
	            success: function (result){
	               switch (result){
	                    case 0 : alert("<spring:message code='app.modify.alert1' />" );
	                      break ;
	                    case 1 : alert("<spring:message code='app.modify.alert2' />")
	                      break ;
	                    case 2 : alert("<spring:message code='app.modify.alert3' />");
	                      break;
	                    case 5 : alert("<spring:message code='app.modify.alert4' />")
	                    	break;
	                    case 6 : alert("Mobileprovision is required");
	               }
	            }
	        });
		}else{
			alert("<spring:message code='app.modify.alert5' />")
		}
	});
	
/* 	20180605 : lsy - single app request */
	var appRequestSinglePop
	$('#appRequestSingle, #appRequestNewVersion').click(function(){
		var winWidth = 750;
		var winHeight = 600;
		var winPosLeft = (screen.width - winWidth)/2;
		var winPosTop = (screen.height - winHeight)/2;
		var nowVer = $("#verNum").val();

		var url = "/app/request.html?radioSearch=&searchDate=&searchValue=&dupleVerNum=&dupleCheckYN=&nowVer="+nowVer;
		if($(this).attr('id') == 'appRequestSingle'){
			url = url+"&popupGb=update";
		}else if($(this).attr('id') == 'appRequestNewVersion'){
			url = url+"&popupGb=newVer";
		}
		url = url + "&appSeq=${app.appSeq}" + "&ostype="+$("[name = ostype] option:selected").val();
		var opt = "width=" + winWidth + ", height=" + winHeight + ", top=" + winPosTop + ", left=" + winPosLeft + ", scrollbars=No, resizeable=No, status=No, toolbar=No, location=No";
		
		appRequestSinglePop = window.open(url, "appRequestSinglePop", opt);			
	});
	...
	$('input:radio[name=nonmemDownGb]').each(function(index){
		...
		if($(this).is(':checked')==false){
			...
			if($(this).val() == '1'){
				...
				$(this).parents('span').find('input:text').eq(0).prop('disabled', false);
				$(this).parents('span').find('input:text').eq(1).prop('readonly', true);
				$(this).parents('span').find('input:text').eq(1).prop('disabled', true);
			}
			else{
...
function inappCntVsappContentsAmt(){
	var isValid = false;
	//varcompletGb_  = $('input:radio[name=completGb]:checked').val();		20180411 : lsy - complet_gb radio delete
...
function textItemDisabled(){
	//20180403 - lsy : when app(library type) complet, not user app request
	$('#appRequest').css("display", "none");
	...
	$("#versionCode").attr('readonly', true);
	
	$("[name=templateName]").attr('disabled', true);
	$("[name=descriptionText]").attr('disabled', true);
	$("[name=chgText]").attr('disabled', true);

	$("[name=templateName]").css({"background-color":"#EEEEEE"});
	$("[name=descriptionText]").css({"background-color":"#EEEEEE"});
	$("[name=chgText]").css({"background-color":"#EEEEEE"});
	//20180409 : lsy - app complet, not modify info - end
}
...
<div id="wrap" class="sub_wrap">
	...
	<div id="container">
		...
		<form name="appFrm" method="get" action="/app/list.html" >
			...
			<input type="hidden" name="isAvailable" id="isAvailable" value="${param.isAvailable }" />
			<input type="hidden" name="appSeq" id="appSeq" value="${app.appSeq }">
			<input type="hidden" name="regGb" value="${app.regGb }" />
			<input type="hidden" name="storeBundleId " value="${app.storeBundleId }" />
		</form>
		<form name="appForm" id="appForm" method="post" enctype="multipart/form-data" action="" >
			...
			<input type="hidden" name="completGb" id="completGb" value="${app.completGb }" />
		...
		<div id="deleteFileList" style="display:none;">
			...
			<div class="section fisrt_section">
				<div class="table_area">
					<table class="rowtable write_table">
						<colgroup>
						...
						<tr>
							...
							<td>
								<input type="hidden" name="templateSeq" value="${app.templateSeq }"/>
								<input name="templateName" type="text" style="width:78%;" class="line_right" value="${app.templateName } (${templateVO.verNum})">
						...
						<tr>
							...
							<td>
								<div class="radio_area">
									<input name="loginGb" id="loginGb_1" type="radio" value="1" <c:if test="${'1' eq appVO.loginGb }">checked="checked"</c:if>> <label for="loginGb_1"><spring:message code='app.modify.text30' /></label>
									<input name="loginGb" id="loginGb_2" type="radio" value="2" <c:if test="${'2' eq appVO.loginGb }">checked="checked"</c:if>> <label for="loginGb_2"><spring:message code='app.modify.text31' /></label>
									<%-- <a href="#" id="use_user_pop" class="btn btnL btn_gray_light"><spring:message code='template.modify.029' /></a> --%>
								</div>
								<div class="time_area" >
									<label for="logoutTime"> <spring:message code='extend.local.086' /></label> &nbsp;&nbsp;&nbsp;&nbsp;<input style="width:50px; text-align:right;" type="text" name="logoutTime" id="logoutTime" value="${app.logoutTime }"><spring:message code='extend.local.085' />
									&nbsp;&nbsp;
									<label for="loginTime"> <spring:message code='extend.local.087' /></label> &nbsp;&nbsp;<input style="width:50px; text-align:right;" type="text" name="loginTime" id="loginTime" value="${app.loginTime }"><spring:message code='extend.local.085' />
								</div>
							</td>
						</tr>
						<tr style="border-bottom : thin dotted #000000;">
							...
							<td>
								<div id="fixDiv" class="radio_area" style="margin:0;">
									<input name="useUserGb" id="useUserGb_1" type="radio" value="1" <c:if test="${app.useUserGb == 1}">checked="checked"</c:if> > <label style="margin-right:100px;" for="useUserGb_1"><spring:message code='template.modify.027' /></label>
									<input name="useUserGb" id="useUserGb_2" type="radio" value="2" <c:if test="${app.useUserGb == 2}">checked="checked"</c:if> > <label for="useUserGb_2"><spring:message code='extend.local.077' /><%-- <span id="useCnt" style="margin-right:5px">(${UserCnt}<spring:message code='template.modify.028_2' />)</span> --%></label>
							...
							<td>
								<div class="radio_area coupon_area" style="width:100%">
									...
									<span>
										...
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input id="d_start_date" name="memDownStartDt" type="text" title="start date" class="date fmDate1" value="<fmt:formatDate value="${app.memDownStartDt}" pattern="yyyy-MM-dd"/>" />
										&nbsp;&nbsp;~&nbsp;&nbsp;
										<input id="d_end_date" name="memDownEndDt" type="text" title="end date"   class="date toDate1" value="<fmt:formatDate value="${app.memDownEndDt}" pattern="yyyy-MM-dd"/>" />
									</span>
									<br>
									<span>
							...
							<td>
								<div class="radio_area coupon_area" style="width:100%">
									...
									<span style="width:100%;">
										...
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input id="c_start_date" name="nonmemDownStarDt" type="text" title="start date" class="date fmDate2" value="<fmt:formatDate value="${app.nonmemDownStarDt}" pattern="yyyy-MM-dd"/>" />
										&nbsp;&nbsp;~&nbsp;&nbsp;
										<input id="c_end_date" name="nonmemDownEndDt" type="text" title="end date"   class="date toDate2" value="<fmt:formatDate value="${app.nonmemDownEndDt}" pattern="yyyy-MM-dd"/>" />
									</span>
									<br>
									<span style="width:100%;">
				...
				<c:if test="${'3' eq appVO.completGb }">
					<div class="btn_area_bottom tCenter">
				...
				<c:if test="${'4' eq appVO.completGb }">
					<div class="btn_area_bottom tCenter">
						<a id="distributeReq" href="#distributeReq" class="btn btnL btn_gray_light"><spring:message code='app.modify.text78' /></a>
				...
				<div class="btn_area_bottom tCenter" id="modifyDeleteSection">
					...
					<a id="cancleBtn" href="#cancleBtn" class="btn btnL btn_gray_light"><spring:message code='app.modify.text50' /></a>
...
<%-- 					<c:choose>
					...
					<c:if test="${'2' eq appVO.regGb }">
						...
						<!-- 20180607 : lsy - app create request -->
						<c:if test="${'1' eq appVO.completGb }">
							<a id="appRequestNewVersion" href="#appRequestNewVersion" class="btn btnL btn_gray_light"><spring:message code='app.modify.text83' /></a>
						</c:if>
						<c:if test="${'2' eq appVO.completGb || '4' eq appVO.completGb }">
							<!-- 20180605 : lsy - single app request -->
							<a id="appRequestSingle" href="#appRequestSingle" class="btn btnL btn_gray_light"><spring:message code='app.modify.text82' /></a>
						</c:if>
						<!-- 20180607 : lsy - app create request - end -->
					</c:if>
				</div>
			</div>
...
$(document).ready(function(){alert("test");
	var useGbTrIndex = $('[name=useGb]').parents('tr').index();
		$('.rowtable.write_table').find('tr').each(function(index){
