...
*/			//20180528 : lsy - lsy - 그룹권한 추가(기존권한 삭제) > 기존 로직 변경
	...
	$("#withDrawalBtn").click(function(){
		//message : 정말 탈퇴하시겠습니까?
		if(confirm("<spring:message code='my_page.modify.048' />")){
			...
			$.ajax({
				...
				success:function(result){
					switch(result){
						//message : 탈퇴 오류
						case 0 : alert("<spring:message code='my_page.modify.046' />");
							break;
						//message : 탈퇴 되었습니다.
						case 1 : alert("<spring:message code='my_page.modify.047' />");
	...
	$("#my_page_modify_f").validate({
	    ...
	    messages: {
	    	//message : 성을 입력해 주십시오
	    	firstName: "<spring:message code='my_page.modify.026' />",
	    	//message : 이름을 입력해 주십시오
			lastName: "<spring:message code='my_page.modify.025' />",
			userPw: {
				//message : 8글자 이상 입력해주셔야 합니다.
				minlength : "<spring:message code='my_page.modify.030' />",
				//message : 영문, 숫자 및 특수기호를 조합하여 등록하셔야 합니다.
				specialChar : "<spring:message code='my_page.modify.031' />"
			},
			...
			userPwConfirm: {
				...
				equalTo: "<spring:message code='my_page.modify.032' />"
			},
			
			phone : {
				//message : 전화번호를 입력해 주십시오.
				required : "<spring:message code='my_page.modify.033' />",
				//message : 숫자만 입력해 주십시오.
				number : "<spring:message code='my_page.modify.035' />"
			},
			
			email:{
				//message : 메일을 입력해 주십시오.
				required : "<spring:message code='my_page.modify.040' />",
				//message : 메일 형식이 적당하지 않습니다.
				email : "<spring:message code='my_page.modify.041' />"
			},
			
			//message : 기업명을 입력해 주십시오.
			companyName : "<spring:message code='my_page.modify.038' />",
			
			companyTel : {
				//message : 대표전화(회사전화번호)를 입력해주십시오.
				required : "<spring:message code='my_page.modify.039' />",
				//message : 숫자만 입력해 주십시오.
				number : "<spring:message code='my_page.modify.035' />"
			} 
		},
	...
	$("#emailBtn").on("click", function(){
		...
		if(email.length == 0){
			//message : 메일을 입력해 주십시오.
			alert("<spring:message code='my_page.modify.040' />");
		}
		else{
			if($("#email-error:visible").length){
				//message : 메일 형식이 적당하지 않습니다.
				alert("<spring:message code='my_page.modify.041' />");
			}
			else if(email === "<c:out value='${memberVO.email}'/>"){
				//message : 지금 사용하고 있는 메일입니다.
				alert("<spring:message code='my_page.modify.045' />");
			}else{
				$.ajax({
					...
					success:function(result){
						switch(result){
							...
							case 0 : alert("<spring:message code='my_page.modify.042' />");
								$("#emailValidFlag").prop("value",1);
								$("#userStatus").prop("value","5");
								break;
							//message : 해당 메일이 이미 존재합니다.
							case 1 : alert("<spring:message code='my_page.modify.043' />");
								break;
							//message : [심각] 해당 메일이 2개 이상 존재하거나, DB Error 발생
							case 2 : alert("<spring:message code='my_page.modify.044' />");
								break;
						}
					}
				});
	...
	$("#my_page_modify_f").on("submit", function(event){
		if($("#emailValidFlag").val() == "0" && $("#isUseMail").val() == "true"){
			event.preventDefault();
			//message : 메일 중복 확인해 주십시오
			alert("<spring:message code='my_page.modify.024' />");
			$("#email").focus();
		}
...
//				if("<c:out value='${memberVO.email}'/>" != $("#email").val() && '${memberVO.userGb}' != '1' && $("#email").val().length > 0){
				if("<c:out value='${memberVO.email}'/>" != $("#email").val() && $("#email").val().length > 0){
					...
					var result = confirm("<spring:message code='my_page.modify.020' />");
					if(result == true){
						$("#preventSubmit").val("1");	
					}else{
						event.preventDefault();
	...
	$("#my_pageModifyBtn").click(function(e){
		if($("#preventSubmit").val() == 0){

			$("#my_page_modify_f").submit();
		}else{
			e.preventDefault();
		}
	});
	...
	if("<c:out value='${modifySuccess}'/>" == "true"){
		//message : 수정되었습니다.
		alert("<spring:message code='my_page.modify.049' />");
	}
});
...
<div id="wrap" class="sub_wrap">
	...
	<div id="container">
		<div class="contents join_area">
			...
			<!-- my_page header -->
			<%@ include file="../inc/my_page_header.jsp" %>
			
			<!-- join_area -->
			<h2><spring:message code='my_page.modify.001' /></h2>
			<a href="#" id="withDrawalBtn" name="withDrawalBtn" class="btn btnM btnM_gray_light"><spring:message code='my_page.modify.002' /></a>
			<form action="/my_page/modify.html" method="post" name="my_page_modify_f" id="my_page_modify_f" class="my_page_modify_f">
			<div class="section first_section">
				<div class="table_area">
					<table class="rowtable write_table">
						...
						<tr>
							<th scope="row"><label class="title" for="userId"><em>*</em> <spring:message code='my_page.modify.003' /></label></th>
							<td colspan="3">
								<input id="userId" name="userId" type="text" value="${memberVO.userId }"style="width:95.7%;" readonly>
							</td>
						</tr>
						<tr>
							<th scope="row"><label class="title" for="userPw">&nbsp;&nbsp;&nbsp;<spring:message code='my_page.modify.004' /></label></th>
							<td colspan="3">
								<input id="userPw" name="userPw" type="password" style="width:95.7%;">
							</td>
						</tr>
						<tr>
							<th scope="row"><label class="title" for="userPwConfirm">&nbsp;&nbsp;&nbsp;<spring:message code='my_page.modify.005' /></label></th>
							<td colspan="3">
								<input id="userPwConfirm" name="userPwConfirm" type="password" style="width:95.7%;">
							</td>
						</tr>
						<tr>
							<th scope="row"><label class="title" for="lastName"><em>*</em> <spring:message code='my_page.modify.007' /></label></th>
							<td>
								<input id="lastName" name="lastName" type="text" value="${memberVO.lastName}" style="width:82%;">
							</td>
							<th scope="row"><label class="title" for="firstName"><em>*</em> <spring:message code='my_page.modify.008' /></label></th>
							<td>
								<input id="firstName" name="firstName" type="text" value="${memberVO.firstName}"style="width:87%;">
							</td>
						</tr>
						<tr>
							<th scope="row"><label id="test" class="title" for="email"><em>*</em> <spring:message code='my_page.modify.009' /></label></th>
							<td colspan="3">
								<input id="email" name="email" type="email" value="${memberVO.email}" style="width:79%;" class="line_right">
								<a href="#" id="emailBtn" class="btn btnL btn_gray_light line_left"><spring:message code='my_page.modify.010' /></a>
							</td>
						</tr>
						<tr>
							<th scope="row"><label class="title" for="phone"><em>*</em> <spring:message code='my_page.modify.011' /></label></th>
							<td colspan="3">
			...
			<div class="section second_section">
				<h3><spring:message code='my_page.modify.012' /></h3>
				<div class="table_area">
					<table class="rowtable write_table">
						<colgroup>
							<col style="width:150px">
						...
						<tr>
							<th scope="row"><label class="title" for="companyName"><em>*</em> <spring:message code='my_page.modify.013' /></label></th>
							<td>
								<input id="companyName" name="companyName" type="text" value="${companyVO.companyName}" style="width:94.7%;">
							</td>
						</tr>
						<tr>
							<th scope="row"><label class="title" for="companyTel"><em>*</em> <spring:message code='my_page.modify.014' /></label></th>
							<td>
								<input id="companyTel" name="companyTel" type="text" value="${companyVO.companyTel}" style="width:94.7%;">
							</td>
						</tr>
						<tr>
							<th scope="row"><label class="title" for="zipcode"> <spring:message code='my_page.modify.015' /></label></th>
							<td colspan="3">
								<input id="zipcode" name="zipcode" type="text" value="${companyVO.zipcode}" style="width:94.7%;">
							</td>
						</tr>
						<tr>
							<th scope="row"><label class="title" for="addrFirst"> <spring:message code='my_page.modify.016' /></label></th>
							<td>
								<input id="addrFirst" name="addrFirst" type="text" value="${companyVO.addrFirst}" style="width:95%;">
							</td>
						</tr>
						<tr>
							<th scope="row"><label class="title" for="addrDetail"> <spring:message code='my_page.modify.017' /></label></th>
							<td>
			...
			<div class="btn_area_bottom tCenter">
				<a href="#" id="my_pageModifyBtn" class="btn btnL btn_red"><spring:message code='my_page.modify.018' /></a>
